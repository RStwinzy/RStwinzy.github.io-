# RStwinzy.github.io-
"My Portfolio"

**Home/About Me**
  Hi! I’m Riddhi Arun Raval, an 18-year-old computer diploma student passionate about  programming languages, software solutions, and designing user-friendly interfaces. I love turning ideas into functional and beautiful digital experiences.
  
 **Skills**

**Programming:** HTML, C, JavaScript, Python
**Frameworks:**  React, Flask
**UI/UX Tools:** Figma, Canva
**Other Tools:** Git, VS Code, WordPress

---

 **Projects**
 
 **1. E-commerce App UI/UX Design (Design Project)**
 **Role:** UI/UX Designer
 **Tools Used:** Figma, Canva
 **Problem:** “Designed a mobile app for users to easily browse and buy products.”
 **Solution:** “Created a clean, minimal interface with clear navigation, interactive prototypes, and smooth user flow.”
 **Outcome:**

  * Screenshots of main pages
![Uploading Screenshot (4).png…]()
![Uploading Screenshot (5).png…]()
![Uploading Screenshot (6).png…]()
![Uploading Screenshot (9).png…]()
![Uploading Screenshot (8).png…]()


  * Clickable prototype link (Figma share link)
           https://www.figma.com/proto/Q9q59WyhC9T4RVbVDl5zDb?node-id=0-1&t=NlE9iBzmgfmRsgMs-6
**2. “Task Management Web Application for productivity and organization”**
  **Objective:**   To create a simple and efficient To-Do List web app for managing daily tasks with a modern UI and data persistence.

### **Contact Me**

```
Email: rstwinzy@gmail.com
LinkedIn: linkedin.com/in/yourprofile
GitHub: RStwinzy.github.io
Figma: ravalriddhisiddhiraval@gmail.com
```

---

